...
;; TODO: support all aspects from slynk-backend:describe-symbol-for-emacs:
...
(defun symbol-kinds (symbol)
  ...
  (remove-if-not 'symbolp (slynk-backend:describe-symbol-for-emacs symbol)))
...
(defun macro-properties (symbol &optional shallow)
  (list (cons :name symbol)
        ...
        (cons :arglist (slynk-backend:arglist symbol))
        (cons :package (symbol-package symbol))
        (cons :type :macro)
...
(defun variable-properties (symbol &optional shallow)
  (list (cons :name symbol)
        ...
        (cons :type :variable)
        ;; TODO: fix me
	;; The underlying function, find-source-location, doesn't exist in either
	;; Swank or Slynk
...
(defun %ensure-external (symbol)
  (let ((name (cond ((symbolp symbol)
                    ...
                    (t
                     ...
                     nil))))
    (when (slynk-api:symbol-external-p name)
      symbol)))
